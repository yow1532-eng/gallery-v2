...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhkcWdyZmxyeGpvY25qa3dhY2R3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk2ODczNzgsImV4cCI6MjA5NTI2MzM3OH0.ZcYkdE21-HYVyW1J7xlkFToFjWO6XcdcAGYsqE6W7cE';
const { createClient } = supabase;
const sb = createClient(SUPABASE_URL, SUPABASE_KEY);

let posts = [], selectedThumbIdx = 0, editingIdx = 0, deleteTargetId = null, editingPostId = null;
let pendingFiles = []; let croppedThumbBlob = null; 
...
async function handleFiles(files) {
  uploadingMsg.style.display = 'block'; uploadingMsg.textContent = '최적화 중...';
  ...
  for (const f of Array.from(files)) {
    ...
    if (++loaded === total) {
      ...
      fileInput.value = ''; uploadingMsg.style.display = 'none'; refreshPreviewGrid();
    }
  }
}
...
function refreshPreviewGrid() {
  ...
  document.getElementById('bottomImageCropBtn').style.opacity = pendingFiles.length ? '1' : '0.5';
  document.getElementById('bottomThumbCropBtn').style.opacity = pendingFiles.length ? '1' : '0.5';
}
...
function renderGrid() {
  ...
  posts.forEach(post => {
    ...
    const eBtn = document.createElement('button'); eBtn.textContent = '✏️ 수정'; eBtn.onclick = e => { e.stopPropagation(); editingPostId = post.id; openEditPost(post); };
